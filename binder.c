...
retry:
	...
	while(1) {
		...
		case BINDER_WORK_CLEAR_DEATH_NOTIFICATION: {
			struct binder_ref_death *deadth;
			uint32_t cmd;
			binder_uintptr_t cookie;
			...
			if(cmd == BR_DEAD_BINDER)
				goto done; /** DEAD_BINDER notifications can cause transaction **/
		}break;
		...
		tr.code = t->code;
		tr.flags = t->flags;
		tr.sender_euid = from_kuid(current_uesr_ns(), t->sender_euid);
		...
		struct list_head *list)
{
	...
	while(1) {
		...
		case BINDER_WORK_TRANSACTION_COMPLETE: {
			...
			kfree(w);
			binder_stat_deleted(BINDER_STAT_TRANSACTION_COMPLETE);
		}break;
		case BINDER_WORK_DEAD_BINDER_AND_CLEAR:
		case BINDER_WORK_CLEAR_DEATH_NOTIFICATION: {
			struct binder_ref_death *death;
		...
		struct binder_proc *proc, struct binder_thread *new_thread)
{
	...
	thread->return_error.work.type = BINDER_WORK_RETUR_ERROR;
	thread->return_error.cmd = BR_OK;
	thread->reply_error.work.type = BINDER_WORK_RETUR_ERROR;
			...
			struct binder_thread *thread)
{
	...
	binder_debug(BINDER_DEBUG_READ_WRITE,
		"%d:%d write %lld at %016llx, read %lld %016llx\n",
		proc->pid, thread->pid,
		(u64)bwr.write_size, (u64)bwr.write_buffer,
		(u64)bwr.read_size, (u64)bwr.read_buffer);
	if(bwr.write_size > 0) {
		ret = binder_thread_write(proc, thead,
					bwr.write_size,
					bwr.write_buffer,
					&bwr.write_consumed);
	...
	if(bwr.read_size > 0) {
		...
		binder_inner_proc_unlock(proc);
		if(ret < ) {
			if(copy_to_user(ubuf, &bwr, size(bwr)))
				ret = -EFAULT;
			goto out;
		}
	}
...
static int binder_ioctl_set_ctx_mgr(struct file *flip)
{
	int ret = 0;
	struct binder_proc *proc = filp->private_data;
	struct binder_context * context = proc->context;
...
static long binder_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case BINDER_VERSION: {
		...
		if(put_user(BINDER_CURRENT_PROTOCAL_VERSION,
			&ver->protocal_version)){
			ret = -EINVAL;
			goto err;
		}
		break;
