...
/**
** Note: the case when user wants us to deregister(with NULL as pointer)
** and he isn't currently owner of notification, will be silently discraded.
** It isn't explicilty defined in the POSIx.
**/
...
retry:
	...
	if(notification == NULL) {
	...
	} else if (inode->notify_owner != NULL) {
		ret = -EBUSY;
	} else {
		...
		info->i_atime = inode->i_ctime = current_time(inode);
