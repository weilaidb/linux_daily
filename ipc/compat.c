	...
	u32, third, compat_uptr_t, ptr, u32, fifth)
{
	...
	case SEMOP:
		...
		return sys_settimedop(first, compat_ptr(ptr), second, NULL);
	case SEMTIMEDOP:
		return compat_sys_settimedop(first, compat_ptr(ptr), second,
					compat_ptr(fifth));
	case SEMGET:
	...
	case MSGSND: {
		...
		return do_msgbuf(first, type, up->mtext, second, third);
	}
	case MSGRCV: {
...
COMPAT_SYSCALL_DEFINE4(semctl, int, second, int , semnum, int, cmd, int,arg)
{
...
static inline get_compat_msqid64(struct msqid64_ds *m64,
				struct compat_msqid64_ds __user *up64)
{
	int err;
...
static inline int get_compat_msqid(struct msqid64_ds *m,
					struct compat_msqid64_ds __user *up)
{
	int err;
