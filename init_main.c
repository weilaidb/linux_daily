				...
				const char *unused, void *arg)
{
	...
	if(obsolete_checkset(param))
		return 0;
	
	/** Unused module parameter **/
	if(strchr(param, '.') && (!val || strchr(param, '.') < vals))
		return 0;
...
static int __init init_setup(char *str)
{
	...
	** So we ignore all arguments entered_before_ init=...[MJ]
	**/
...
static noinline void __ref rest_init(void)
{
	...
	** alread, but it's stuck on the kthreadd_done completion.
	**/
...
void __init parse_early_param(void)
{
	...
	/** All fall throught to do_early_param. **/
...
asmlinkage __visible void __init start_kernel(void)
{
	...
	**/
	...
	/**
	** Set up the scheduler prior  starting any iterrupts(such as the 
	** timer interrupt). Full topology setup happends at smp_init()
	...
	** no cause "plain-text" data to be decrypted when accessed.
	**/
	mem_encrypt_init();
...
static bool __init_or_module initcall_blacklisted(initcall_t fn)
{
	...
	add = (unsigned long)dereference_function_descriptor(fn);
...
int __init_or_module do_one_initcall(initcall_t fn)
{
	...
	if(irqs_disabled()) {
		strlcat(msgbuf, "disabled interrupt", sizeof(msgbuf));
		local_irq_enable();
	}
...
extern initcall_t __initcal4_start[];
...
static char *initcall_level_names[] _initdata = {
	"early",
	"core",
	"postcore",
	"arch",
