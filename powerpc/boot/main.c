/**
Updates for PPC64 by Todd Inglett, Dave Engebretesen & Peter Bergner
**/
...
static struct addr_range prep_kernel(void)
{
	...
	**/
	...
	if(platform_ops.vmlinux_alloc) {
		...
		** to an area allocated with malloc()(somewhere past_end).
		**/
	...
	len = partial_decompress(vmlinuz_addr, vmlinuz_size, addr,
				ei.loadsize, ei.elfoffset);
	if(len < 0)
								...
								unsigned long initrd_size)
{
	...
	**/
	...
	if(_initrd_end > _initrd_start) {
		...
		printf("Using load supplied ramdisk at 0x%lx-0x%lx\n\r",
	...
	**/
	...
	if(initrd_addr < vmlinux.size) {
		...
		printf("Relocating initrd 0x%lx <-%p(0x%lx bytes)\n\r",
			initrd_addr, old_addr, initrd_size);
		memmove((void *)initrd_addr, old_addr, initrd_size);
	}
	
	printf("intird head: 0x%lx\n\r", *((unsignd long *)initrd_addr));
...
static char cmdline[BOOT_COMMAND_LINE_SIZE]
		__attribute((__section__("__builtin_cmdline")));
	
static void prep_cmdline(void *chosen)
{
	unsignd int getline_timeout = 5000;
	int v;
	int n;
	...
	/** Put the command line back to the devtree for the kernel */
...
void start(void)
{
	...
	unsignd long ft_addr = 0;
	void *chosen;
	/** Do this first, because malloc() could clobber the loader's
	** command line. Only use the loader command line if a 
	** built-in command line wasn't set by and external tool **/
	...
	chosen = finddevice("/device");
	if(!chosen)
		chosen = create_node(NULL, "chosen");

	vmlinux = prep_kernel();
	initrd = prep_initrd(vmlinux, chosen,
					loader_info.initrd_addr, load_info.intird_size);
	...
	if(ft_addr) {
		...
		kentry((unsignd long)initrd.addr, initrd.size,
			loader_info.promptr);
